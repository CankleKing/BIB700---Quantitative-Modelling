...
############################## Visualizing trends ##############################
### Plotting FIG 4 - Patch size on 
# Define the ranges of patch sizes
size_ranges <- c(1, 21, 41, 61, 81, 101)

# Subset big_results to include only the first 20 timesteps
...
# Linearity
# Create the scatterplot and perform visual check for linearity for each pop
...
plot(model_Q,1)
## ^^ very interestingly if patch size is below 25 populations are nearly 0
## See discussion for more detail, we will continue and smooth the loess line
## by using the loess method instead of the lm to plot the regression line
...
# Extract the P-values for each model
P_N <- coef(summary(model_N))[,4]
P_P <- coef(summary(model_P))[,4]
P_Q <- coef(summary(model_Q))[,4]
...
ggplot(avg_population, aes(x = patch_size)) +
  ...
  geom_smooth(aes(y = N), method = "loess", se = FALSE, color = "black",
              lty = "solid") +
  geom_point(aes(y = P, color = "P"), size = 3, shape = "+") +
  geom_smooth(aes(y = P), method = "loess", se = FALSE, color = "blue", 
              lty = "solid") +
  geom_point(aes(y = Q, color = "Q"), size = 3, shape = "+") +
  geom_smooth(aes(y = Q), method = "loess", se = FALSE, color = "red", 
              lty = "solid") +
